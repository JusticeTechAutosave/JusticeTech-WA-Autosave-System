...
// ─────────────────────────────────────────────────────────────────────────────
// DM-only bot — all group/broadcast messages are dropped at entry.
// Performance fixes:
...
module.exports = async (sock, m, chatUpdate, store) => {
  try {
    ...
    // ── Block all group/community messages — DM-only bot ─────────────────────
    ...
    const ctx = {
      ...
      botNumber, botJid, senderNumber, senderJid,
      version: VERSION,
    };
    ...
    // ── Command plugins ───────────────────────────────────────────────────────
    if (isCmd && command) {
      // ── Maintenance gate — block all non-dev commands when maintenance is ON ─
      const _maint = global.__JT_MAINTENANCE;
      if (_maint?.active && !isDev && command !== "maintenance" && command !== "maint") {
        const userMsg = _maint.message || "The bot is currently undergoing scheduled maintenance.";
        const lines = [
          "🔧 *Bot Maintenance In Progress*", "",
          "*" + userMsg + "*", "",
        ];
        if (_maint.startTime || _maint.endTime) {
          lines.push("─────────────────────");
          if (_maint.startTime) lines.push("🕐 *Start Time :* " + _maint.startTime);
          if (_maint.endTime)   lines.push("🕑 *End Time   :* " + _maint.endTime);
          lines.push("─────────────────────", "");
        }
        lines.push(
          "⏳ All bot features are temporarily unavailable.",
          "✅ You will be notified automatically when we're back online.", "",
          "We apologize for the inconvenience. — *JusticeTech Team*"
        );
