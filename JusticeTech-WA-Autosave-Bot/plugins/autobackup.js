...
async function doBackupRun(botNum, ownerJid, sock) {
  ...
  try {
    ...
    console.log(`[autobackup] ✅ Done for ${botNum}`);
  } catch (e) {
...
function startOwnerTimer(botNum, ownerJid, sock, intervalMinutes) {
  ...
  global.__AB_TIMERS[botNum] = setInterval(() => doBackupRun(botNum, ownerJid, sock), ms);
...
module.exports = {
  ...
  run: async (ctx) => {
    ...
    if (callerIsDev && sub === "force") {
      ...
      await doBackupRun(targetNum, `${targetNum}@s.whatsapp.net`, sock);
    ...
    if (!sub || sub === "status" || sub === "show") {
      ...
      return reply(
        ...
        `${p}autobackup now              — run immediately`
      );
    }
    ...
    if (sub === "now") {
      ...
      await doBackupRun(botNum, ownerJid, sock);
      return reply("✅ Backup done. Check your configured delivery destinations.");
    }
