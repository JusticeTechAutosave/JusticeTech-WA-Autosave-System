...
async function runBackup(sock, ownerJid, botNum, overrideEmail, silent = false) {
  ...
  // NOTE: Backup ONLY goes to the bot owner who triggered it.
  // Do NOT send to all premium users — the restore code is private.
  ...
  // 2. WA DM — only on manual backup, NOT on silent scheduled autobackup
  if (!silent && bkCfg.waBackup && sock && recipientJids.size > 0) {
    let waOk = false;
    let waErr = null;
    ...
    results.wa = { ok: false, skipped: silent ? "silent-autobackup" : !bkCfg.waBackup };
  }
