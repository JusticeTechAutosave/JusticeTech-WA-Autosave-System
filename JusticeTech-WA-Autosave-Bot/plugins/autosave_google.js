...
function readWelcomeDM() {
  ...
  const w = readJson(WELCOME_FILE, { dmText: "" });
...
module.exports = {
  ...
  run: async ({ sock, m, botNumber, botJid, store, args, command, prefix, reply, isOwner, isPremium }) => {
    try {
      ...
      const DEV_ONLY_CMDS   = new Set([]); // no autosave commands are dev-only anymore
      ...
      if (cmd === "autosave") {
        ...
        if (!sub || sub === "status") {
          ...
          return safeReply(
            { sock, m, reply },
            `⚙️ *Autosave Status*\n\n` +
            `Enabled : ${f.enabled ? "✅ YES — actively saving contacts" : "❌ NO — paused"}\n` +
            `Updated : ${f.updatedAt || "-"}\n\n` +
            `Use:\n${pfx}autosave on  — activate autosave\n${pfx}autosave off — pause autosave`
          );
        }

        if (sub === "on") {
          const f = readJson(AUTOSAVE_FLAG_FILE, { enabled: true });
          if (f.enabled) return safeReply({ sock, m, reply }, "ℹ️ Autosave is already ON.");
          ...
          return safeReply(
            ...
            `• Ask for their name\n` +
            `• Save them to Google Contacts automatically\n\n` +
            `Check status: ${pfx}autosave status`
          );
        }

        if (sub === "off") {
          const f = readJson(AUTOSAVE_FLAG_FILE, { enabled: true });
          if (!f.enabled) return safeReply({ sock, m, reply }, "ℹ️ Autosave is already OFF.");
          ...
          return safeReply(
            ...
            `• Start the autosave flow for new contacts\n\n` +
            `⚠️ Already saved contacts are unaffected.\n` +
            `To turn back on: ${pfx}autosave on`
          );
        }
