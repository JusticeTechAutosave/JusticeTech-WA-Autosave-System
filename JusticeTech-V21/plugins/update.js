...
module.exports = {
  name: "Update",
  category: "system",
  desc: "Auto-update the bot from the latest GitHub release (dev only)",
  command: ["update"],
  devOnly: true,

  run: async ({ reply, args, isDev, prefix }) => {
    if (!isDev) {
    ...
    if (sub === "changelog") {
      ...
      try {
        const remote = await fetchLatestRelease();
        const body   = remote.body.slice(0, 3000); // WA message limit
        return reply(
    ...
    if (!newer && !forced) {
      return reply(
        ...
        `Use ${pfx}update force if you want to reinstall anyway.`
      );
    }
