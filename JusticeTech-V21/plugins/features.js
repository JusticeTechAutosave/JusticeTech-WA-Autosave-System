...
module.exports = {
  ...
  run: async ({ reply, args, m, prefix, isPremium, isOwner, isDev }) => {
    const all = Array.isArray(global.PLUGINS) ? global.PLUGINS : [];
    const p   = prefix || ".";
    const ver = global.BOT_VERSION || "v1.1.1 JT";

    const sub = String(args?.[0] || "").toLowerCase().trim();

    // ── .features all — dev sees EVERYTHING ───────────────────────────────────
    if (sub === "all") {
      if (!isDev) return reply("🔒 Developer-only. Use .features free / .features premium / .features owner instead.");
    ...
    if (sub === "owner" || sub === "admin") {
      if (!isOwner && !isDev) return reply("🔒 Owner-only view.");
